...
# Upload Documents
uploaded_files = st.file_uploader("Upload your documents (PDFs, DOCs, EPUBs)", accept_multiple_files=True, type=["pdf", "docx", "epub"])
...
def get_embeddings(text_list):
    response = co.embed(model='embed-english-v2.0', texts=text_list)
...
def process_documents(uploaded_files):
    ...
    try:
        ...
        for file in uploaded_files:
            ...
            if document_chunks:
                logging.info(f"Extracted {len(document_chunks)} chunks from {file.name}.")
            else:
...
def search_faiss(query, k=5):
    ...
    return [st.session_state['document_chunks'][i] for i in indices[0]]
...
def generate_answer(query, retrieved_texts):
    ...
    response = co.generate(
        model='command-xlarge-nightly',  # Or use 'command-medium-nightly'
        prompt=prompt,
        max_tokens=300,
        temperature=0.5,
    )
...
if st.session_state['index'] is not None:
    ...
    query = st.text_input("Ask me anything:", key=f"input_{st.session_state['input_key']}")
